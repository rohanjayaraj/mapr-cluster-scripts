    ...
    if  [ -s "opt/mapr/lib/libMapRClient.so.1" ] && [ -s "/opt/mapr/lib/libMapRClient.so.1" ]; then
        ...
        if [ -n "$setupclient" ]; then
            ...
            for file in $files; do
                ...
                [ -n "$asanso" ] && sed -i "/ \"\$JAVA\"/i  export LD_PRELOAD=${asanso}" $file
            done
            ...
            for file in $files; do
                ...
                [ -n "$asanso" ] && sed -i "/^[[:space:]]*\$JAVA /i  export LD_PRELOAD=${asanso}" $file
            done
            ...
            for file in $files; do
                ...
                [ -n "$asanso" ] && sed -i "/^[[:space:]]*[nohup]*[[:space:]]*java /i  export LD_PRELOAD=${asanso}" $file
...
function maprutil_runDiskTest(){
    ...
    # Build summary report if running `--direct`
    if [ -n "${direct}" ]; then
        local readMB="$(cat ${disktestdir}/*.log | grep "cached reads" | awk '{print $5}' | sort -n)"
        #local stdDev=$(echo "${readMB}" | awk '{sum+=$1; sumsq+=$1*$1}END{printf("%d",sqrt(sumsq/NR - (sum/NR)**2))}')
        local meanMB=$(echo "${readMB}" | awk '{sum+=$1;}END{printf("%d",sum/NR)}')
        #local midMB=$(echo "${meanMB}-${stdDev}" | bc)
        local highMB=$(echo "${readMB}" | tail -n 1)

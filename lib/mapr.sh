...
function maprutil_startTraces() {
    if [[ "$ISCLIENT" -eq "0" ]] && [[ -e "/opt/mapr/roles" ]]; then
        ...
        nohup sh -c 'log="/opt/mapr/logs/mfstop.log"; rc=0; while [[ "$rc" -ne 137 && -e "/opt/mapr/roles/fileserver" ]]; do mfspid=`pidof mfs`; if [ -n "$mfspid" ]; then timeout 10 top -bH -p $mfspid -d 1 >> $log; rc=$?; else sleep 10; fi; sz=$(stat -c %s $log); [ "$sz" -gt "629145600" ] && tail -c 1048576 $log > $log.bkp && rm -rf $log && mv $log.bkp $log; done' > /dev/null 2>&1 &
        ...
        nohup sh -c 'log="/opt/mapr/logs/gatewaytop.log"; rc=0; while [[ "$rc" -ne 137 && -e "/opt/mapr/roles/gateway" ]]; do gwpid=$(cat /opt/mapr/pid/gateway.pid 2>/dev/null); if kill -0 ${gwpid}; then timeout 10 top -bH -p $gwpid -d 1 >> $log; rc=$?; else sleep 10; fi; sz=$(stat -c %s $log); [ "$sz" -gt "1258291200" ] && tail -c 1048576 $log > $log.bkp && rm -rf $log && mv $log.bkp $log; done' > /dev/null 2>&1 &
    fi
...
function maprutil_buildMFSCpuUse(){
    ...
    local fsthreads="$(echo "$mfsthreads" | grep CpuQ_FS | cut -d':' -f2- | sed 's/,/ /g')"
    ...
    local dbthreads="$(echo "$mfsthreads" | grep CpuQ_DBMain | cut -d':' -f2- | sed 's/ //g' | sed 's/,/ /g')"
    ...
    local dbhthreads="$(echo "$mfsthreads" | grep CpuQ_DBHelper | cut -d':' -f2- | sed 's/ //g' | sed 's/,/ /g')"
    ...
    local dbfthreads="$(echo "$mfsthreads" | grep CpuQ_DBFlush | cut -d':' -f2- | sed 's/ //g' | sed 's/,/ /g')"
    ...
    local compthreads="$(echo "$mfsthreads" | grep CpuQ_Compress | cut -d':' -f2- | sed 's/ //g' | sed 's/,/ /g')"
